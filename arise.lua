...
task.spawn(function()
	while task.wait(0.1) do
		if autoFarm and not areEnemiesAlive() and not isTweening then
			...
			if doorPos then
				for i = 1, 5 do
					hrp.CFrame = CFrame.new(doorPos + Vector3.new(0, -7, 0))
					task.wait(0.01)
				end
				roomIndex = newIndex
			end
...
local expectedPlaceID = 128336380114944  -- ID đúng, nếu đúng thì vô hiệu hóa
local correctID = 1 -- Thay bằng ID dungeon đúng nếu cần
...
-- Toggle: Auto Start Dungeon (chỉ hoạt động khi sai ID)
	...
	Callback = function(state)
		if state then
			if game.PlaceId ~= expectedPlaceID then
				...
				print("Auto Dungeon bị vô hiệu hoá vì đang ở đúng Place ID:", expectedPlaceID)
			end
		end
	end
})
